...
/**
 * This pattern finds and moves children of the root node which are not clusters under the appropriate adoptive cluster-node. An orphan
 * would be placed under the cluster-node with the largest number of children which point to the orphan.
 * 
 * Tie-breakers are solved by checking the targets of the orphan.
 * 
 * If cluster-nodes A and B are competing for the orphan, that which has the largest number of children being targets of the orphan, wins
 * it.
 *
 */
		...
		for (int j = 0; j < vRootChildren.size(); j++) {
			...
			if (!ncurr.isCluster()) {
				...
				for (Object target : targets)
				{
					...
					/**********************************************************************************
					 * NOTE: retain only targets of the orphan which are clusters <-- only the cluster which is lowest in the tree should
					 * adopt the orphan,
					 *********************************************************************************/
					...
					if (curr_target.getLevel() > 1 && !ncurr_target.isCluster())
					{
						...
						if (nparent.isCluster())
						{
							...
							if (ht.containsKey(nparent))
							{
								 ...
								 * Vector(dvalues); for(int j=0; j<ivalues.size(); j++) { ivalue = (Double)ivalues.elementAt(j);
							...
							else
							{
								...
								/* IO.put("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"); dvalues = ht.values(); ivalues = new
								 * Vector(dvalues); for(int i=0; i<ivalues.size(); i++) { ivalue = (Double)ivalues.elementAt(i);
								 * IO.put("Value of Hashtable **** " + ivalue.doubleValue()); }
								 * IO.put("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"); */
							}
						}
				...
				if (!ht.isEmpty()) {
					...
					/* Collection cvalues = ht.values(); Vector values = new Vector(cvalues); for(int i=0; i<values.size(); i++) { Double
					 * value = (Double)values.elementAt(i); IO.put("Value of Hashtable **** " + value.doubleValue()); } */
		...
		for (int j = 0; j < vRootChildren.size(); j++) {
			...
			if (!ncurr.isCluster()) {
				...
				for (Object source : sources)
				{
					...
					/**********************************************************************************
					 * NOTE: retain only sources of the orphan which are clusters <-- only the cluster which is lowest in the tree should
					 * adopt the orphan,
					 *********************************************************************************/
					...
					if (curr_source.getLevel() > 1 && !ncurr_source.isCluster())
					{
						...
						DefaultMutableTreeNode parent = (DefaultMutableTreeNode) curr_source.getParent();
						Node nparent = (Node) parent.getUserObject();
						// IO.put("::::::: whose parent is :=  ");
