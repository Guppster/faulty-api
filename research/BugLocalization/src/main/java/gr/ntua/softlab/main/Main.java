...
public class Main{
	...
	private void performRanking(){
		...
		temp.addAll(fileRsfMaker.getAllRelations().get("filebelongstomodule").keySet());
		int c = 0;
		for (String s : temp)
			if (!s.contains(".h"))
				c++;
	...
	private void outputURanked(){
				...
				for (String s : entityToEntities.getValue()) {
					if (finalV.contains(s))
						...
						fileToCliqueConnectivity
								.put(entityToEntities.getKey(), fileToCliqueConnectivity.get(entityToEntities.getKey()) + 1);
				}
			}
		}
		...
		try {
			...
			BufferedWriter URankedWriter = new BufferedWriter(new FileWriter(Paths.URANKEDREPORTS + productName + "/"
	...
	private void performClusterRanking(){
					...
					if (list.indexOf(cluster) > 0.5 * list.size()
						...
						firstLevelWriter
								.write(String.format(
										...
										cluster.getValue(),
										cluster.getValue()
												* (solutionClusterName2FileName.get(cluster.getKey()).size() / (double) solutionFileName2ClusterName
														.size()), averagePerFileCon.get(cluster.getKey())
												* solutionClusterName2FileName.get(cluster.getKey()).size(),
										averageToCliqueCon.get(cluster.getKey())
												* solutionClusterName2FileName.get(cluster.getKey()).size(), solutionClusterName2FileName
												.get(cluster.getKey()).size(), cluster.getValue() / max));
						System.out
		...
		try {
			...
			for (Entry<String, Set<String>> finalSelectedCluster : top50PercentileClusters.entrySet()) {
				...
				ZeroWriter
						.write(String.format("______________________________------------------------------___________________________\n"));
				System.out.println("______________________________------------------------------___________________________");
			}
			ZeroWriter.close();
		} catch (IOException ioe) {
		...
		try {
			...
			BufferedWriter BigWriter = new BufferedWriter(new FileWriter(Paths.BIGREPORTS + productName + "/"
					+ currentFileName.split("_")[0] + ".csv"));
				...
				for (Entry<String, Double> filetoCliqueCluster : fileToCliqueClusterConnectivity.entrySet()) {
					if (finalSelectedCluster.getValue().contains(filetoCliqueCluster.getKey())) {
						if (inClusterRanking.containsKey(filetoCliqueCluster.getValue())) {
							...
							inClusterRanking.put(filetoCliqueCluster.getValue(), new ArrayList<String>());
							inClusterRanking.get(filetoCliqueCluster.getValue()).add(filetoCliqueCluster.getKey());
						}
					}
				}

				Map<Double, List<String>> treeMap1 = new TreeMap<Double, List<String>>(new Comparator<Double>(){
	...
	private Map<String, Set<String>> rearrangeClusters(Set<String> clustersToBreakDown, Set<String> clique){
		...
		for (String clusterName : clustersToBreakDown) {
			for (String orphanName : solutionClusterName2FileName.get(clusterName)) {
				orphansToClustersConnections.put(orphanName, new HashMap<String, Integer>());
	...
	private void printSolutionRsf(Set<String> SolutionSet, Map<String, Map<String, Set<String>>> fileRsfIn, String Bug_ID,
		...
		try {
			BufferedWriter solutionWriter = new BufferedWriter(new FileWriter(new File(Paths.SOLUTION_ACDC_INPUT + Bug_ID
					+ "_sol_final_acdc.rsf")));
			for (Entry<String, Map<String, Set<String>>> completeRelation : fileRsf.entrySet()) {
				for (Entry<String, Set<String>> fromEntityToEntity : completeRelation.getValue().entrySet()) {
					for (String toEntity : fromEntityToEntity.getValue()) {
						// if (fromEntityToEntity.getKey().contains("sqlmeta.cpp") || toEntity.contains("sqlmeta.cpp"))
						// System.out.println("From " + fromEntityToEntity + " To " + toEntity);
						if (SolutionSet.contains(fromEntityToEntity.getKey()) && SolutionSet.contains(toEntity)
								&& !bad.contains(fromEntityToEntity.getKey()) && !bad.contains(toEntity))
							solutionWriter.write(completeRelation.getKey() + " " + entityToId.get(fromEntityToEntity.getKey()) + " "
		...
		try {
			...
			while (solutionClusterReader.ready()) {
				...
				if (solutionClusterName2FileName.containsKey(clusterName)) {
					...
					solutionClusterName2FileName.put(clusterName, new HashSet<String>());
					solutionClusterName2FileName.get(clusterName).add(fileName);
				}
					...
					if (validFiles.contains(entityToEntities.getKey()) && validFiles.contains(toEntity)) {
						if (goodClusters.contains(solutionFileName2ClusterName.get(entityToEntities.getKey())))
							if (weights.containsKey(solutionFileName2ClusterName.get(entityToEntities.getKey()))) {
								if (weights.get(solutionFileName2ClusterName.get(entityToEntities.getKey())).containsKey(
										solutionFileName2ClusterName.get(toEntity))) {
									Integer cur = weights.get(solutionFileName2ClusterName.get(entityToEntities.getKey())).get(
											solutionFileName2ClusterName.get(toEntity));
									cur += 1;
									weights.get(solutionFileName2ClusterName.get(entityToEntities.getKey())).put(
											solutionFileName2ClusterName.get(toEntity), cur);
								} else {
									weights.get(solutionFileName2ClusterName.get(entityToEntities.getKey())).put(
											solutionFileName2ClusterName.get(toEntity), new Integer(1));
								}
							} else {
								weights.put(solutionFileName2ClusterName.get(entityToEntities.getKey()), new HashMap<String, Integer>());
								weights.get(solutionFileName2ClusterName.get(entityToEntities.getKey())).put(
										solutionFileName2ClusterName.get(toEntity), new Integer(1));
							}
						else if (weights.containsKey(solutionFileName2ClusterName.get(toEntity))) {
							if (weights.get(solutionFileName2ClusterName.get(toEntity)).containsKey(
									solutionFileName2ClusterName.get(entityToEntities.getKey()))) {
								Integer cur = weights.get(solutionFileName2ClusterName.get(toEntity)).get(
										solutionFileName2ClusterName.get(entityToEntities.getKey()));
								cur += 1;
								weights.get(solutionFileName2ClusterName.get(toEntity)).put(
										solutionFileName2ClusterName.get(entityToEntities.getKey()), cur);
							} else {
								weights.get(solutionFileName2ClusterName.get(toEntity)).put(
										solutionFileName2ClusterName.get(entityToEntities.getKey()), new Integer(1));
							}
						} else {
							weights.put(solutionFileName2ClusterName.get(toEntity), new HashMap<String, Integer>());
							weights.get(solutionFileName2ClusterName.get(toEntity)).put(
									solutionFileName2ClusterName.get(entityToEntities.getKey()), new Integer(1));
						}
					}
	...
	private Set<String> selectClusters(Map<String, Map<String, Integer>> clusterConnectionWeights, Set<String> inputClusters){
		...
		for (Entry<String, Integer> clusterWeight : perClusterGain.entrySet()) {
			// System.out.println(clusterWeight.getKey() + " : " + clusterWeight.getValue());
	...
	private Set<String> selectMostConnected(Map<String, Map<String, Set<String>>> fileRsfIn, Set<String> candidateForConnection){
		...
		for (String s : candidateForConnection)
			if (!s.endsWith(".h"))
				finalVi.add(s);
		System.out.println("Starting with " + candidateForConnection.size() + " files, after removing .h I am left with " + finalVi.size()
				+ " files");
	...
	private void toFromCliqueConnectivity(Map<String, Map<String, Set<String>>> fileRsfIn, Set<String> clique, Set<String> GSClusters){
		...
		for (Entry<String, Integer> cluster : cliqueClusterConnectivity.entrySet()) {
			averageToCliqueCon.put(cluster.getKey(), cluster.getValue()
