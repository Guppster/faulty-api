...
public class FPGrowth {
	...
	public FPGrowth(File file, int threshold, String productName)
	...
	private FPtree conditional_fptree_constructor(
			Map<String, Integer> conditionalPatternBase,
			Map<String, Integer> conditionalItemsMaptoFrequencies,
			int threshold, Vector<FPtree> conditional_headerTable) {
		// FPTree constructing
		// the null node!
	...
	private void preProcessing(File file,
			...
			List<String> sortedItemsbyFrequencies, Vector<String> itemstoRemove)
			throws FileNotFoundException {
		while (input.hasNext()) {
		...
		while (input.hasNextLine()) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				for (String vectorString : transactionSortedbyFrequencies) {
					...
					// equals situatioans
					if (itemsMaptoFrequencies.get(vectorString) < itemsMaptoFrequencies
							.get(item)
							|| ((itemsMaptoFrequencies.get(vectorString) == itemsMaptoFrequencies
									.get(item)) && (vectorString
									.compareToIgnoreCase(item) < 0 ? true
									: false))) {
			...
			/*
			 * for (String vectorString : transactionSortedbyFrequencies) {
			 * System.out.printf("%-10s%10s ", vectorString,
			 * itemsMaptoFrequencies.get(vectorString)); } System.out.println();
			 */
			// printing transactionSortedbyFrequencies
			/*
			 * for (String vectorString : transactionSortedbyFrequencies) {
	...
	void insert(Vector<String> transactionSortedbyFrequencies, FPtree fptree,
			Vector<FPtree> headerTable) {
	...
	private void fpgrowth(FPtree fptree, int threshold,
			Vector<FPtree> headerTable) {
	...
	void FPgrowth(FPtree fptree, String base, int threshold,
			Vector<FPtree> headerTable, Map<String, Integer> frequentPatterns) {
		for (FPtree iteminTree : headerTable) {
			String currentPattern = (base != null ? base : "")
					+ (base != null ? " " : "") + iteminTree.item;
			...
			while (iteminTree.next != null) {
				...
				while (!conditionalItem.isRoot()) {
					conditionalPattern = conditionalItem.item
							+ " "
							+ (conditionalPattern != null ? conditionalPattern
									: "");
			...
			for (String itemsforTable : conditionalItemsMaptoFrequencies
					.keySet()) {
			...
			FPtree conditional_fptree = conditional_fptree_constructor(
			...
			Collections.sort(conditional_headerTable,
					new frequencyComparitorinHeaderTable());
			//
			if (!conditional_fptree.children.isEmpty()) {
				FPgrowth(conditional_fptree, currentPattern, threshold,
						conditional_headerTable, frequentPatterns);
			}
		}
	}

	private void insert(Vector<String> pattern_vector, int count_of_pattern,
			FPtree conditional_fptree, Vector<FPtree> conditional_headerTable) {
		...
		insert(pattern_vector, count_of_pattern, newNode,
	...
	private void print() throws FileNotFoundException {
		 ...
		 * sortedItems.add("null"); frequentPatterns.put("null", 0); for (String
		 * item : frequentPatterns.keySet()) { int count =
		 * frequentPatterns.get(item);
		 * 
		 * int i = 0; for (String listItem : sortedItems) { if
		 * (frequentPatterns.get(listItem) < count) { sortedItems.add(i, item);
		 * break; } i++; } }
		 */
		Formatter output = new Formatter("../../frequent_" + productName
				+ ".out");
		for (String frequentPattern : frequentPatterns.keySet()) {
			output.format("%s\t%d\n", frequentPattern,
					frequentPatterns.get(frequentPattern));
		}
		output.close();
	}
}
